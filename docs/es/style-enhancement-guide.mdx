---
title: Guía de mejoras de estilos
...
1. **Definiciones de tokens** — propiedades personalizadas CSS con valores para modo claro/oscuro
2. **Mapeo de componentes** — qué componentes del tema utilizan cada token
3. **Notas de implementación** — orientación de uso y consideraciones de accesibilidad

Las tablas de tokens están divididas por **modo claro** y **modo oscuro** donde los valores difieren.
...
| `--f5-surface-hover` | `var(--f5-white-2)` | Relleno en hover |
...
| `--f5-surface-hover` | `var(--f5-black-3)` | Relleno en hover |
...
El tema utiliza **sombras alfa con tinte neutro** con valores de doble capa para una profundidad natural. El modo claro usa `--f5-black-3` (`#343434`) como base de tinte; el modo oscuro usa `#cccccc`.
...
### Mapeo de sombras por componente
...
| Tarjetas al hacer hover | `--f5-shadow-mid` |
...
:root {
  ...
  --f5-radius-sm: 0.3125rem;   /* 5px — elementos de nav, controles pequeños */
  --f5-radius-md: 0.375rem;    /* 6px — tarjetas, bloques de código */
  --f5-radius-lg: 0.5rem;      /* 8px — modales, contenedores grandes */
  --f5-radius-full: 999px;     /* píldoras, totalmente redondeado */
}
```

### Mapeo de radio por componente
...
| Elementos de nav de la barra lateral | `--f5-radius-sm` (5px) |
...
## 4. Estilos de navegación de la barra lateral

### Estilos de elementos de la barra lateral
...
nav.sidebar a[aria-current="page"] {
  ...
  padding-left: calc(0.5rem - 3px);  /* compensar el borde */
}
```
...
```css
/* Primario — usando F5 Red como acción de marca */
.btn-primary {
...
| Tamaño | Padding | Tamaño de fuente | Altura mínima |
|---|---|---|---|
...
> **Estado**: Completo — todas las variantes de botón (`primary`, `secondary`, `tertiary`, `critical`) y escala de tamaños (`sm`, predeterminado, `lg`) implementadas en `styles/custom.css`.

---

## 6. Fondos de héroe y encabezado
...
```css
/* Degradado de héroe primario — azul River */
...
/* Desvanecimiento inferior hacia el fondo de página */
.hero-fade::after {
  content: '';
  position: absolute;
  bottom: 0;
...
> **Estado**: Completo — degradados de héroe (`primary`, `eggplant`, `red`, `faint`) y superposiciones (`hero-fade`, `hero-vignette`) implementados en `styles/custom.css`.
...
| `--f5-transition-spring` | `0.2s` | `cubic-bezier(0.54, 1.5, 0.38, 1.11)` | Tooltips, entrada de popovers |

### Qué animar

| Propiedad | Seguro para animar | Notas |
|---|---|---|
| `background-color` | Sí | Estándar para rellenos en hover |
| `color` | Sí | Cambios de color de texto |
| `border-color` | Sí | Énfasis de borde al hacer hover |
| `box-shadow` | Sí | Cambios de elevación (usar `will-change` si hay problemas de rendimiento) |
| `transform` | Sí | Escala, traslado para retroalimentación |
| `opacity` | Sí | Aparecer/desaparecer con fundido |
| `width`, `height` | Evitar | Causa reflujo de diseño |
| `padding`, `margin` | Evitar | Causa reflujo de diseño |

### Mapeo de transiciones por componente

```css
/* Elementos de nav de la barra lateral */
nav.sidebar a {
  transition: background-color var(--f5-transition-fast),
              color var(--f5-transition-fast);
}

/* Tarjetas — elevación de sombra al hacer hover */
.swatch, .icon-card {
...
El tema utiliza un patrón de **doble anillo** con un anillo interno insertado y un brillo exterior:

```css
/* Enfoque de acción — usando F5 River azul */
:root {
...
- El patrón de doble anillo cumple los requisitos de **Apariencia de enfoque WCAG 2.2** (contorno de contraste mínimo de 2px)
- El uso de `box-shadow` en lugar de `outline` permite respetar el radio de borde
- `:focus-visible` garantiza que el anillo solo aparezca para la navegación con teclado, no con clics del ratón
...
| Display 100 | `0.8125rem` (13px) | 1.385 | Texto de encabezamiento |
| Body 300 | `1rem` (16px) | 1.5 | Cuerpo principal |
| Body 200 | `0.875rem` (14px) | 1.429 | Texto de barra lateral, subtítulos |
| Body 100 | `0.8125rem` (13px) | 1.385 | Letra pequeña |

### Características principales

| Aspecto | Valor | Notas |
|---|---|---|
| Altura de línea de encabezados | `1.1` | Encabezados ajustados para impacto de marca; considerar `1.2` para legibilidad |
...
| Pesos de encabezados | `h1-h2: 700, h3: 500, h4-h6: 700 uppercase` | Opinado para coherencia de marca |

> **Prioridad**: No se necesitan cambios. La tipografía actual está bien definida y es apropiada para la marca. El ajuste menor de altura de línea de encabezados (`1.1` a `1.2`) es opcional.

---

## 10. Registro de cambios de implementación

Todos los tokens de diseño se implementaron en cinco sprints:
...
| 4. Degradados de héroe | Utilidades de fondo | `.hero-gradient-primary`, `-eggplant`, `-red`, `-faint`, `.hero-vignette`, `.hero-fade` |
| 5. Enfoque + Transiciones | Accesibilidad + acabado | `--f5-focus-action`, `--f5-focus-critical`, `--f5-transition-*` (5 tokens), efectos hover de tarjetas |
