...
title: DNS
description: >-
  Diagramas de resolução DNS incluindo resolução recursiva, GSLB e gerenciamento
  de DNS do F5 XC.
sidebar:
  order: 14
i18n:
...
Diagramas de arquitetura DNS abrangendo fluxos de resolução recursiva, balanceamento de carga global de servidores e gerenciamento de DNS do F5 Distributed Cloud.

## Fluxo de Resolução DNS

Resolução padrão de consultas DNS do cliente através do resolver recursivo até o servidor de nomes autoritativo com integração GSLB.
...
## Gerenciamento de DNS do F5 XC

Gerenciamento de DNS do F5 Distributed Cloud fornecendo balanceamento de carga DNS inteligente entre origens multi-cloud.
...
## Arquitetura de Balanceamento de Carga DNS

Balanceamento de carga DNS em múltiplas camadas com roteamento geográfico, verificações de integridade e failover entre regiões de nuvem.
