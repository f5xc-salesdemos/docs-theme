...
테마는 `assets/` 디렉터리에 두 개의 로고 파일을 제공합니다:
...
기본 로고는 `config.ts`의 `createF5xcDocsConfig` 팩토리에 설정되어 있습니다:
...
Starlight는 라이트 모드와 다크 모드에 대한 별도 로고를 지원합니다:
...
- **`viewBox` 포함** — 로고가 비례적으로 확대/축소되도록 보장합니다 (예: `viewBox="0 0 100 100"`)
- **파일 크기를 작게 유지** — 로고는 모든 페이지에서 로드되므로 10 KB 미만을 목표로 합니다
- **두 테마에서 테스트** — 라이트 모드와 다크 모드 모두에서 로고가 올바르게 표시되는지 확인합니다
