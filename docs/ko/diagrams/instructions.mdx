---
title: 지침
description: 'Mermaid 다이어그램 구문 참조, 사용 가능한 아이콘 팩, CSS 스타일링 및 테마 호환성 세부 정보.'
sidebar:
  order: 2
i18n:
...
이 테마에는 펜스형 ` ```mermaid ` 코드 블록을 `<div class="mermaid-container">` 래퍼로 변환하는 커스텀 `remark-mermaid` 플러그인(`plugins/remark-mermaid.mjs`)이 포함되어 있습니다. Mermaid CDN 스크립트는 페이지 로드 시 SVG 다이어그램을 렌더링합니다.
...
`architecture-beta` 다이어그램 유형은 아이콘이 적용된 노드를 통해 서비스 및 그룹 정의를 지원합니다:
...
다음 아이콘 팩이 등록되어 있으며 Mermaid 다이어그램에서 사용할 수 있습니다. 아이콘은 참조될 때에만 CDN에서 지연 로드됩니다.
...
Mermaid 다이어그램은 둥근 모서리와 레이어드 박스 그림자를 가진 스타일 컨테이너로 래핑됩니다:
...
Mermaid 래퍼에서 Starlight의 기본 `<pre>` 스타일링이 제거되어 외부 컨테이너 테두리만 표시됩니다:
...
내부 SVG는 다크 모드 호환성을 위해 흰색 배경으로 강제 설정됩니다:
...
- `.mermaid-container`는 다크 모드에서 흰색 SVG 배경을 사용합니다
- 컨테이너 테두리는 `--sl-color-gray-5`를 사용합니다
- 컨테이너는 0.75rem 테두리 반경과 레이어드 박스 그림자를 가집니다
- 다이어그램은 라이트 및 다크 테마 모두에서 가독성이 유지됩니다
- Mermaid CDN 스크립트가 로드되어 페이지 로드 시 SVG를 렌더링합니다
