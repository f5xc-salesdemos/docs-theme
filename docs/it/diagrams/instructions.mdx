...
description: >-
  ...
  disponibili, stili CSS e dettagli sulla compatibilità dei temi.
sidebar:
  order: 2
i18n:
...
Il tema include un plugin personalizzato `remark-mermaid` (`plugins/remark-mermaid.mjs`) che converte i blocchi di codice ` ```mermaid ` delimitati in wrapper `<div class="mermaid-container">`. Lo script CDN di Mermaid esegue il rendering dei diagrammi SVG al caricamento della pagina.

## Sintassi delle icone

### Diagrammi di flusso

Utilizzare la sintassi nodo `@\{\}` per aggiungere icone da qualsiasi pacchetto di icone registrato:
...
I seguenti pacchetti di icone sono registrati e disponibili nei diagrammi Mermaid. Le icone vengono caricate in modo lazy dal CDN solo quando referenziate.
...
I diagrammi Mermaid sono racchiusi in un contenitore stilizzato con angoli arrotondati e un'ombra del box a livelli:
...
- `.mermaid-container` ha lo sfondo SVG bianco in modalità scura
- Il bordo del contenitore utilizza `--sl-color-gray-5`
- Il contenitore ha un raggio del bordo di 0.75rem e un'ombra del box a livelli
- I diagrammi sono leggibili sia nei temi chiari che in quelli scuri
- Lo script CDN di Mermaid si carica e renderizza gli SVG al caricamento della pagina
